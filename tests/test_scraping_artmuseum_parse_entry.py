...
from murmansk_culture_api.datatypes import ArtmuseumTimeLabel
from murmansk_culture_api.db.models import ArtmuseumExhibition
from murmansk_culture_api.scraping.artmuseum import parse_entry


def html_template(
    title="default title",
    url="default url",
    time_label=ArtmuseumTimeLabel.NOW,
    ...
    if time_label == ArtmuseumTimeLabel.NOW:
        label_class = "label_now"
        label = "Сейчас"
    elif time_label == ArtmuseumTimeLabel.SOON:
...
def test_parse_dates():
    ...
    entry_now = html_template(
        time_label=ArtmuseumTimeLabel.NOW, start_date=start_date, end_date=end_date
    )
    entry_soon = html_template(
        time_label=ArtmuseumTimeLabel.SOON, start_date=start_date, end_date=end_date
    )
...
def test_parse_without_end_date():
    ...
    entry_now = html_template(time_label=ArtmuseumTimeLabel.NOW, start_date=start_date)
    entry_soon = html_template(
        time_label=ArtmuseumTimeLabel.SOON, start_date=start_date
    )
...
def test_parse_entry():
    ...
    assert parse_entry(entry) == ArtmuseumExhibition.parse_obj(args)
